...
def run(project_locations="eopod/", current_head="eopod", base_head=None):
    """
...
def get_rst_filename(module_dotted, fallback):
    if module_dotted:
        fname = module_dotted.replace("eopod.", "").replace("src.", "")
...
def create_rst(name, children, output_dir, module_dotted=None):
    ...
    Parameters:
      ...
      - module_dotted: The full module dotted name (e.g. "eopod.callib.triton_call").
